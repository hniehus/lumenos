...
smoke: run
	...
	@grep -q "cap: invalid access code=1" $(BOOTLOG)
	@grep -q "cap: wrong-type access code=2" $(BOOTLOG)
	@grep -q "cap: insufficient-rights access code=4" $(BOOTLOG)
	@grep -q "cap: attenuation cycles complete count=1000" $(BOOTLOG)
